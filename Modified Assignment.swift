                                ...
                                if isNotEditing {
                                    ...
                                    if row == .clear{
                                        ...
                                        if operatorType == .plus {
                                            let result = (tempNumber + (Double(totalNumber) ?? 0))
                                            totalNumber = String(format: "%.2f", result)
                                        } else if operatorType == .minus {
                                            let result = (tempNumber - (Double(totalNumber) ?? 0))
                                            totalNumber = String(format: "%.2f", result)
                                        } else if operatorType == .multiply {
                                            let result = ((Double(totalNumber) ?? 0) * tempNumber)
                                            totalNumber = String(format: "%.2f", result)
                                        } else if operatorType == .divide {
                                            let result = (tempNumber / (Double(totalNumber) ?? 1)) // 0으로 나누는 경우를 방지하기 위해 1로 대체
                                            totalNumber = String(format: "%.2f", result)
                                        }
                                    }
